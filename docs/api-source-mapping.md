...
|---|---|---|---|---|---|---|
| 자치구 인구 | 성별/연령/총인구 | `district_age_gender_population.csv` fallback | 기본: 주민등록인구 동별 `OA-877`, 구별 `OA-12240`; 병행: `SPOP_LOCAL_RESD_DONG`, `OA-14991`, `OA-14992` | 주 1회 웹DB 또는 월 1회 | 높음 | 주민등록인구를 기본 표시, 생활인구를 전환 버튼으로 병행 표시 |
| 도서관 반경 인구 | 행정동별 성별/연령/총인구 | `2_population_and_senior.csv`, `3_gender.csv` fallback | 기본: 주민등록인구 동별 `OA-877`; 병행: `SPOP_LOCAL_RESD_DONG`, `OA-14991`, `OA-14992` | 주 1회 웹DB | 높음 | 반경 2km 행정동 집계 결과를 주민등록인구/생활인구 모두 사전 로드 |
